package com.cs.tripjournal

import android.os.Bundle
import androidx.appcompat.app.AppCompatActivity
import com.google.android.gms.maps.CameraUpdateFactory
import com.google.android.gms.maps.GoogleMap
import com.google.android.gms.maps.OnMapReadyCallback
import com.google.android.gms.maps.SupportMapFragment
import com.google.android.gms.maps.model.CameraPosition
import com.google.android.gms.maps.model.LatLng
import com.google.android.gms.maps.model.MarkerOptions


class CountryPlace : AppCompatActivity(), OnMapReadyCallback {

    private lateinit var mMap: GoogleMap
    var citylongitude =0.0
    var citylatitude =0.0
    var cityname =""
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_country_place)
        citylongitude =intent.getDoubleExtra("CITY_LONGITUDE",0.0)
        citylatitude =intent.getDoubleExtra("CITY_LATITUDE",0.0)
        // Obtain the SupportMapFragment and get notified when the map is ready to be used.
        cityname =intent.getStringExtra("CITY_NAME")
        val mapFragment = supportFragmentManager
            .findFragmentById(R.id.map) as SupportMapFragment
        mapFragment.getMapAsync(this)

    }

    /**
     * Manipulates the map once available.
     * This callback is triggered when the map is ready to be used.
     * This is where we can add markers or lines, add listeners or move the camera. In this case,
     * we just add a marker near Sydney, Australia.
     * If Google Play services is not installed on the device, the user will be prompted to install
     * it inside the SupportMapFragment. This method will only be triggered once the user has
     * installed Google Play services and returned to the app.
     */

    override fun onMapReady(googleMap: GoogleMap) {
        mMap = googleMap

        // Add a marker in Sydney and move the camera
        val place = LatLng(citylatitude,citylongitude)

        mMap.addMarker(MarkerOptions().position(place).title(cityname))

        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(place,16f))
    }

}
